...
* 邮件通知发送逻辑应检查用户在 `/profile/settings` 保存的邮件通知偏好。
* 邮件通知发送失败只记录日志，不影响评论、回复、支付或购买权限主流程。

## Next Steps

邮件系统下一阶段建议：

* 上线前确认 Resend 域名验证、额度、退信和垃圾箱表现。
* 生产环境设置 `SITE_URL` 为正式域名，确保邮件里的作品链接正确。
* 新作品发布邮件通知暂不做同步群发；后续如接入，应使用分批或队列策略。
* 如果后续切换 SMTP 或其他服务，优先替换 `lib/email.ts` 内部 provider 实现，业务 API 仍复用 `sendEmail`。
